...
void odomCallback(const nav_msgs::Odometry::ConstPtr& msg)
{
    ...
    quat_est[0] = msg->pose.pose.orientation.x;
    quat_est[1] = msg->pose.pose.orientation.y;
    quat_est[2] = msg->pose.pose.orientation.z;
    quat_est[3] = msg->pose.pose.orientation.w;
}
...
int main(int argc, char **argv)
{
  ...
  ros::Publisher publ_input = nh.advertise<mobile_control::motorMsg>("/input_msg",1);

  ros::Subscriber sub1 = nh.subscribe("/ns1/cmd_msg",1,cmdCallback);
  ros::Subscriber sub2 = nh.subscribe("/camera/odom/sample",1,odomCallback);
...
// Distance between camera and center : 296 mm
  double d_cam_to_center = 0.296;
...
// Motor specification and unit conversion factors //
  ...
  while(ros::ok())
  {
    ...
    x_est[1] = pose_est[0]+d_cam_to_center*(1-cos(phi_est[0]));
    y_est[1] = pose_est[1]+d_cam_to_center*sin(phi_est[0]);
