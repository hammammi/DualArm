...
float pid_controller(float e1){
	float kp = 2*3;
	float ki = 1.5*6.2;
	float kd = 0.0002;
	float dt = 1.0/1000.0;
	float G[3] = {kp+ki*dt+kd/dt,-(kp+2*kd/dt), kd/dt};
	error[0] = e1;
	float pid = pre_pid + G[0]*error[0] + G[1]*error[1] + G[2]*error[2];
	pre_pid = pid;
	error[2] = error[1];
	error[1] = error[0];
	return pid;
...
int des_value_trajectory(float input,float ref){

	float dxl_goal_value;
	float offset;


    dxl_goal_value = pid_controller(ref-input);

            	
    return radtopulse(dxl_goal_value);
}
...
void EPOS_CSP(void *arg)
{
    ...
    while (run)
    {
	    ...
	    float sin_max = 0.025;
	    if (gt > 61.0) sine_amp = sin_max - sin_max*((gt-61.0)/120.0);
	    else sine_amp = sin_max;

            if (sine_amp < 0) sine_amp = 0;
	    if (gt > 1.0) reference=(float) (sine_amp*(sin(PI2*f*(gt-1.0)))); //(sine_amp*(sin(PI2*f*gt)))
	    else reference = 0.0;

	   /*float ramp_max = 0.1;
	    if (gt < 1.0) ival = 0;
	    else if(gt < 16.0){
			reference = (float) (ramp_max*((gt-1.0)/5));
			if (reference > ramp_max) reference = ramp_max;
		}
	    else if (gt<31.0){
			reference = (float) (-ramp_max*((gt-16.0)/5)+ramp_max);
			if (reference < -ramp_max) reference = -ramp_max;
		}
	    else {
			reference = (float) (ramp_max*((gt-31.0)/5)-ramp_max);
			if (reference > 0) reference = 0;
		}
	   */
	    //reference = 0.02; 
	    q = float(int(serial_pt.Data2)-int(mag_offset[1]))/(16384.0)*2.0*M_PI;
            ival = des_value_trajectory(q,reference);
	    printf("%d %0.3f\n",ival,q);
           
	    if (ival >= 35000) ival = 35000;
	    if (ival <= -35000) ival = -35000;
