...
class tracking_controller{
    ...
    void cmd_vel_pub(){
        ...
        if(sqrt(pow((goal_p.x_goal-est_pos_vel.x_est),2) + pow((goal_p.y_goal-est_pos_vel.y_est),2))<0.001 && fabs(goal_p.phi_goal - est_pos_vel.phi_est)<0.001 || init_pos == true){
            ...
            cmd_vel_acc.ax_cmd = 0;
            cmd_vel_acc.ay_cmd = 0;
            cmd_vel_acc.aphi_cmd = 0;
    ...
    // Controller gain
    const double Kp[3] = {0,0,0};
    const double Kd[3] = {0,0,0};
