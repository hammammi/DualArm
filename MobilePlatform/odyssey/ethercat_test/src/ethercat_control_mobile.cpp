            ...
            rt_printf("%d slaves found and configured.\n", ec_slavecount);
                ...
                for (int k=1; k<(NUMOFWHEEL_DRIVE+1); ++k)
                {
                    if (( ec_slavecount >= 1 ) && (strcmp(ec_slave[k].name,"EPOS4") == 0)) //change name for other drives
                    {
                        ...
                        wkc_count=ec_SDOwrite(k,0x6085, 0x00, FALSE, os, &ob, EC_TIMEOUTRXM);
    ...
    while (run)
    {
        ...
        if (sys_ready)
        {
...

            {
                ...
                rt_printf("\n");
                if(epos4_drive_pt[i].ptInParam->ErrorCode == 0x8280 || epos4_drive_pt[i].ptInParam->ErrorCode == 0x8250)
                {
                    faultreset_CtrlWrd(epos4_drive_pt[i].ptInParam->StatusWord, &controlword);
                }
            }
        } // sysready
        else
...
int main(int argc, char** argv)
{
    ...
    rt_task_create(&pub_task, "publish_task", 0, 70, 0 );
