...
int32_t zeropos[NUMOFEPOS4_DRIVE-1] = {0}; // initial pos
int32_t homepos[NUMOFEPOS4_DRIVE-1] = {-63715, 38594};//, 37694, -20069};
int32_t desinc[NUMOFEPOS4_DRIVE-1] = {0};
int32_t targetpos[NUMOFEPOS4_DRIVE-1] = {-63715};//, 38594, 37694, -20069};
double velprofile[NUMOFEPOS4_DRIVE-1] = {2};//, 2, 2, 2};
double accprofile[NUMOFEPOS4_DRIVE-1] = {100};//, 100, 100, 100};
double c_1[NUMOFEPOS4_DRIVE-1] = {0};
int t1[NUMOFEPOS4_DRIVE-1] = {0};
int t2[NUMOFEPOS4_DRIVE-1] = {0};
    ...
    while (run)
    {
        ...
        if (sys_ready)
        {
            for (i=1; i<NUMOFEPOS4_DRIVE; ++i)
            {
                ...
                {
                    if (targetpos[i-1] >= zeropos[i-1]) {
                        velprofile[i-1] = abs(velprofile[i-1]);
                        accprofile[i-1] = abs(accprofile[i-1]);
                    }
                    else {
                        velprofile[i-1] = -abs(velprofile[i-1]);
                        accprofile[i-1] = -abs(accprofile[i-1]);
                    }
                    if (c_1[i-1] < abs(targetpos[i-1] - zeropos[i-1])) {
                        if (gt >0 && gt <= t1[i-1]) {
                            p_des = (int) (zeropos[i-1] + 0.5 * accprofile[i-1] * gt * gt * rpms2ipss);
                        } else if (gt <= t2[i-1]) {
                            p_des = (int) (zeropos[i-1] + c_1[i-1] + (gt - t1[i-1]) * velprofile[i-1] * rpm2ips);
                        } else if (gt <= (t1[i-1] + t2[i-1])) {
                            p_des = (int) (zeropos[i-1] + c_1[i-1]
                                           + (t2[i-1] - t1[i-1]) * velprofile[i-1] * rpm2ips +
                                           (gt - t2[i-1]) * velprofile[i-1] * rpm2ips
                                           - 0.5 * accprofile[i-1] * rpms2ipss * (gt - t2[i-1]) * (gt - t2[i-1]));
                        } else {
                            p_des = targetpos[i-1];
                        }
                    } else {
                        if (gt > 0 && gt <= t1[i-1]) {
                            p_des = (int) (zeropos[i-1] + 0.5 * accprofile[i-1] * rpms2ipss * gt * gt);
                        } else if (gt <= t2[i-1]) {
                            p_des = (int) (zeropos[i-1] + velprofile[i-1] * t1[i-1]
                                           - 0.5 * accprofile[i-1] * rpms2ipss * (t2[i-1] - gt) * (t2[i-1] - gt));
                        } else {
                            p_des = targetpos[i-1];
                        }
                    }
        ...
        else
        {
            for (i=1; i<NUMOFEPOS4_DRIVE; ++i)
            {
                ...
                targetpos[i-1] = zeropos[i-1];
                c_1[i-1] = velprofile[i-1]*velprofile[i-1]/2.0/accprofile[i-1]*pow(2,18)/60.0;
                epos4_drive_pt[i].ptOutParam->TargetPosition=zeropos[i-1];
                if (c_1[i-1]<abs(targetpos[i-1]-zeropos[i-1])){
                    t1[i-1] = (int) (velprofile[i-1]*rpm2ips/(accprofile[i-1]*rpms2ipss));
                    t2[i-1] = (int) (abs(targetpos[i-1]-zeropos[i-1])/(velprofile[i-1]*rpm2ips));
                }
                else {
                    t1[i-1] = (int) (sqrt(abs(targetpos[i-1] - zeropos[i-1]) / (accprofile[i-1] * rpms2ipss)));
                    t2[i-1] = (int) (2 * t1[i-1]);
                }
            }
        }
...
void motion_callback(const ethercat_test::pos& msg)
{
    for (int i=1; i<NUMOFEPOS4_DRIVE; ++i)
    {
        desinc[i-1] = msg.position[i-1] + homepos[i-1];
    }
    traj_time(desinc);
