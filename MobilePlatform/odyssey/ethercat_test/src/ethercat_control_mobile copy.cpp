            ...
            rt_printf("%d slaves found and configured.\n", ec_slavecount);
                ...
                for (int k=1; k<(NUMOFWHEEL_DRIVE+1); ++k)
                {
                    if (( ec_slavecount >= 1 ) && (strcmp(ec_slave[k].name,"EPOS4") == 0)) //change name for other drives
                    {
                        ...
                        os=sizeof(ob); ob = 0x603F0010;    // ErrorCode  UINT16
...
void EPOS_OP(void *arg)
{
    ...
    ros::Publisher publisher_error_code = n_diagnosis.advertise<motorsMsg>("/error_code",1);
    if (ecat_init()==FALSE)
    {
        run = 0;
        ...
        if (sys_ready)
        {
...

            {
                epos4_drive_pt[i].ptOutParam->TargetVelocity=wheeldes[i];
                    ...
                    if((epos4_drive_pt[i].ptInParam->StatusWord & mask)== 0x08)
                    {
                    os = sizeof(ob3); ob3 = 0;
                    error = ec_SDOread(i,0x1001,0x00,FALSE,&os,&ob,EC_TIMEOUTRXM);
                    rt_printf("%d\n",error);
                        if(i==0) ErrorCode.omega1 = error;
                        else if(i==1) ErrorCode.omega2 = error;
                        else if(i==2) ErrorCode.omega3 = error;
                        else if(i==3) ErrorCode.omega4 = error;
                    }
            
            }

            publisher_error_code.publish(ErrorCode);
        } // sysready
        else
        {
            for (i=0; i<NUMOFWHEEL_DRIVE; ++i)
